package rs.project4420.connect4;

/**
 * Created by enco on 4.11.15..
 *
 * Klasa za cuvanje nekih konstanti
 */
public class Constants {

    public static final int COIN_COLOR_GRID = R.color.grey;
    public static final int COIN_COLOR_PL1 = R.color.red;
    public static final int COIN_COLOR_PL2 = R.color.yellow;

    public static final int COIN_OWNER_GRID = 0;
    public static final int PLAYER_1 = 1;
    public static final int PLAYER_2 = -1;

    public static final int STATUS_DRAW = 0;
    public static final int STATUS_P1_WIN = 1;
    public static final int STATUS_P2_WIN = 2;

    public static final String PLAYER_EXTRA = "player";

    public static final String ARG_TYPE = "type";
    public static final String ARG_ENABLE_TABLE = "enable_table";

    public static final int ARG_FRIEND = 1000;
    public static final int ARG_AI = 1001;
    public static final int ARG_CREATOR_MULTIPLAYER = 1002;
    public static final int ARG_INVITED = 1003;
    public static final String ARG_STATUS_TEXT = "status_text";
}

